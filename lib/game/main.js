...
MyGame = ig.Box2DGame.extend({
	...
	loadLevel: function( level ) {        
		...
		this.player = ig.game.spawnEntity( EntityPlayer , 100, 400 );
	...
	update: function() {
		...
		if( this.edgeBuildTimer >= 0.04 ){
			ig.game.spawnEntity( EntityEdgeShape , this.timeElapsed * 80 , (this.bpm * 3) + 200 );
		...
		for( var i = 0 ; i < ig.game.getEntitiesByType( EntityEdgeShape ).length ; i++ ) {
			...
			//var oldPos = ig.game.getEntitiesByType( EntityEdgeShape )[i].body.GetPosition();
			//var newPos = new Box2D.Common.Math.b2Vec2( oldPos.x -= 1 , oldPos.y ); 
			//ig.game.getEntitiesByType( EntityEdgeShape )[i].body.SetPosition( newPos );
			
			//var reverseVel = ig.game.player.body.GetLinearVelocity().x * -1;
			ig.game.getEntitiesByType( EntityEdgeShape )[i].body.SetLinearVelocity( new Box2D.Common.Math.b2Vec2( -30 , 0) );	
			//remember to delete old edge shapes by xPos
			if( ig.game.getEntitiesByType( EntityEdgeShape )[i].pos.x < ig.game.player.pos.x - 350 ){
