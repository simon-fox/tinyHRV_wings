...
MyGame = ig.Box2DGame.extend({
	...
	loadLevel: function( level ) {        
		...
		this.player = ig.game.spawnEntity( EntityPlayer , 80 , 600 );
	...
	update: function() {
		...
		//sampling rate
		if( this.edgeBuildTimer >= 0.08 ){
			//normalisation for x and y
			ig.game.spawnEntity( EntityEdgeShape , ( this.timeElapsed * 130 ) - 30 , (this.bpm * 3) + 400 );
		...
		if( this.player.pos.x > 1870 ){
			console.log('IT ISS HAPPENING');
			//- clear timeElapsed
			this.timeElapsed = 0;
			//- kill all edgeShapes
			for( var i = 0 ; i < ig.game.getEntitiesByType( EntityEdgeShape ).length ; i++ ) {
			...
			ig.game.player.kill();
			this.player = ig.game.spawnEntity( EntityPlayer , -40 , ( ig.game.bpm * 3 ) + 320 );
			this.player.body.ApplyImpulse( new Box2D.Common.Math.b2Vec2( 300 , -10 ), ig.game.player.body.GetPosition() );

			//var oldPos = ig.game.player.body.GetPosition().y;
			//var oldVel = ig.game.player.body.GetLinearVelocity();
			//ig.game.player.body.SetPosition( new Box2D.Common.Math.b2Vec2( 0 , 60 ) );
