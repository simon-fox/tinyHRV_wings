...
MyGame = ig.Box2DGame.extend({
	...
	loadLevel: function( level ) {        
		...
		this.player = ig.game.spawnEntity( EntityPlayer , 100, 600 );
	...
	update: function() {
		...
		if( this.edgeBuildTimer >= 0.04 ){
			...
			ig.game.spawnEntity( EntityEdgeShape , this.timeElapsed * 80 , (this.bpm * 3) + 400 );
		...
		for( var i = 0 ; i < ig.game.getEntitiesByType( EntityEdgeShape ).length ; i++ ) {
			...
			ig.game.getEntitiesByType( EntityEdgeShape )[i].body.SetLinearVelocity( new Box2D.Common.Math.b2Vec2( -40 , 0) );	
